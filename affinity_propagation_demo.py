...
def create_data(nobs, vert_scale, horz_scale):
    ...
    return ([list(a) for a in zip(x_data, y_data)], [0 for a in range(len(g1_x))] + [1 for a in range(len(g2_x))] + [2 for a in range(len(g3_x))] + [3 for a in range(len(g4_x))])

if __name__ == '__main__':
    # #############################################################################
    # Generate sample data
    data, labels_true = create_data(500,3,1)
    # #############################################################################
    # Compute Affinity Propagation
    af = AffinityPropagation(max_iter=500000, verbose=True).fit(data)
    ...
    colors = cm.colors.rainbow(np.linspace(0, 1, n_clusters_))
    ...
    plt.pdf("affinity-propagation.pdf")
    plt.show()

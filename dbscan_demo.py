...
def dbscan_attempt(X, labels_true, eps):
    # Compute DBSCAN
    db = DBSCAN(eps=eps, min_samples=5, metric='euclidean', algorithm='auto', n_jobs=-1).fit(X)
    ...
    try:
        ...
        print("error occurred for eps: {eps}")
    finally:
        pass
    ...
    plt.title(f"EPS: {eps:0.3f}, Estimated number of clusters: {n_clusters_}")
    plt.savefig(f"dbscan_{eps:0.3f}.pdf")
...
if __name__ == '__main__':
    ...
    for eps in np.linspace(0.005, 0.2, 40):
        dbscan_attempt(X, labels_true, eps)
